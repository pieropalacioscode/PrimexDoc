...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.error || 'Credenciales incorrectas. Intenta nuevamente.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex flex-col justify-center items-center bg-slate-50 px-4 py-8">
      <div className="w-full max-w-md bg-white p-6 sm:p-8 rounded-2xl shadow-sm border border-slate-200">
        ...
        <div className="text-center mb-6">
          ...
          <h1 className="text-2xl font-bold text-slate-800">Inicia Sesión en PrimexDoc</h1>
          <p className="text-sm text-slate-500 mt-1">
            Ingresa a tu plataforma de preparación docente
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          >
            {loading ? 'Iniciando sesión...' : 'Ingresar al Gimnasio'}
          </button>
        </form>
